...
.hero-block {
    background: linear-gradient(90deg, #1a2744 0%, #0f1e3d 100%);
...
.hero-block p { color: #8b9bbf; font-size: 0.95rem; margin: 0; }
...
.metric-stat { display:flex;justify-content:space-between;font-size:0.82rem;color:#8b9bbf;margin:4px 0; }
...
def result_html(titulo, valor):
    ...
    <div class="fatigue-result">
      ...
      <div style="color:#8b9bbf;font-size:0.8rem;">/ 100</div>
      <div class="bar-wrap"><div class="bar-fill" style="width:{pct}%;background:{color}"></div></div>
      <div style="font-size:0.88rem;font-weight:600;color:{color}">{nivel}</div>
      <div style="color:#8b9bbf;font-size:0.82rem;margin-top:4px">{desc}</div>
    </div>"""

# ── HEADER ────────────────────────────────────────────────────────────────────
st.markdown("""
<div class="hero-block">
  <h1>🚴 Predicción de Fatiga en Ciclistas</h1>
  <p>Comparación de modelos · KNN &nbsp;·&nbsp; Regresión Lineal &nbsp;·&nbsp; Árbol de Decisión</p>
</div>
""", unsafe_allow_html=True)
...
if metricas:
    ...
    modelos_info = [
        ...
        (c2, "Regresión Lineal",  "lr",  "LinearRegression · StandardScaler"),
        (c3, "Árbol de Decisión", "dt",  "max_depth=5"),
    ]
    for col, nombre, key, detalle in modelos_info:
        ...
        with col:
            ...
            <div class="metric-card">
              ...
              <div style="color:#8b9bbf;font-size:0.75rem;margin:6px 0 12px">R² — varianza explicada</div>
              ...
              <div style="color:#4b5e82;font-size:0.72rem;margin-top:10px">{detalle}</div>
            </div>""", unsafe_allow_html=True)

st.markdown('<hr class="my-divider">', unsafe_allow_html=True)

# ── INPUTS ────────────────────────────────────────────────────────────────────
st.markdown('<p class="section-label">⚙️ Parámetros del ciclista</p>', unsafe_allow_html=True)

col_a, col_b = st.columns(2)
with col_a:
    bmp         = st.number_input("Frecuencia cardíaca (bpm)",  min_value=0.0, step=1.0, format="%.0f")
...
if predecir:
    ...
    st.markdown('<p class="section-label" style="margin-top:24px">📈 Resultados de predicción</p>', unsafe_allow_html=True)
    r1, r2, r3 = st.columns(3)
    with r1: st.markdown(result_html("KNN", fatiga_knn), unsafe_allow_html=True)
    with r2: st.markdown(result_html("Regresión Lineal", fatiga_lr), unsafe_allow_html=True)
    with r3: st.markdown(result_html("Árbol de Decisión", fatiga_dt), unsafe_allow_html=True)

    promedio = (fatiga_knn + fatiga_lr + fatiga_dt) / 3
    nivel_p, color_p, desc_p = nivel_fatiga(promedio)
    st.markdown(f"""
    <div style="margin-top:20px;background:rgba(59,130,246,0.08);border:1px solid rgba(59,130,246,0.3);
      ...
      <div>
        <div style="font-family:'Space Mono',monospace;font-size:0.7rem;color:#3b82f6;
